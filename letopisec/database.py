...
class Database(ABC):
    ...
    def commit(self, device_uid: int, device_tag: str, records: Sequence[CANFrameRecord]) -> int:
        ...
        Returns the latest recorded seqno for this device_tag after this transaction.
...
class SqliteDatabase(Database):
    ...
    Design notes:
    - Idempotency is enforced by UNIQUE(device_tag_id, seqno); duplicate uploads are normal.
    - `device_tags.last_seqno` is a transactional cache used to return cumulative ACK quickly
      without aggregating over the very large `can_frames` table on each commit.
    - `device_tags.last_device_uid` is operational metadata, not correctness state:
      it lets us detect/log tag->UID changes and track heartbeat-style empty commits cheaply.
    ...
    def commit(self, device_uid: int, device_tag: str, records: Sequence[CANFrameRecord]) -> int:
        if not device_tag:
            raise ValueError("device_tag must be a non-empty string")

        with self._lock:
            LOGGER.debug(
                "Starting commit transaction: device_uid=%d device_tag=%r records=%d",
                device_uid,
                device_tag,
                len(records),
            )
            try:
                ...
                device_tag_id, previous_last_seqno, previous_device_uid = self._ensure_device_tag(
                    cursor=cursor,
                    device_uid=device_uid,
                    device_tag=device_tag,
                )
                if previous_device_uid is not None and previous_device_uid != device_uid:
                    # This does not fail the commit. The tag remains the primary identity on the server side,
                    # while UID drift is surfaced as an anomaly for operators.
                    LOGGER.warning(
                        "Device tag %r changed UID from %d to %d",
                        device_tag,
                        previous_device_uid,
                        device_uid,
                    )
                ...
                if not records:
                    ...
                    # We still refresh last_device_uid for observability and return cached cumulative ACK.
                    cursor.execute(
                        "UPDATE device_tags SET last_device_uid=? WHERE device_tag_id=?",
                        (device_uid, device_tag_id),
                    )
                    cursor.execute("COMMIT")
                    LOGGER.info(
                        "Commit completed with empty payload: device_tag=%r last_seqno=%d",
                        device_tag,
                        previous_last_seqno,
                    )
                ...
                if duplicate_seqnos:
                    LOGGER.warning(
                        "Commit payload for device_tag=%r contains duplicate seqnos: %s",
                        device_tag,
                ...
                LOGGER.debug(
                    "INSERT OR IGNORE finished: device_tag=%r attempted=%d inserted=%d deduplicated=%d",
                    device_tag,
                    len(rows_to_insert),
                    inserted_count,
                ...
                for seqno, records_for_seq in seq_to_records.items():
                    ...
                    if stored is None:
                        LOGGER.error(
                            "Commit verification failed: missing stored row for device_tag=%r seqno=%d",
                            device_tag,
                            seqno,
                        )
                    ...
                    for record in records_for_seq:
                        if not self._record_matches_row(device_uid=device_uid, record=record, row=stored):
                            mismatch_count += 1
                            LOGGER.warning(
                                "Duplicate seqno mismatch for device_tag=%r seqno=%d "
                                "stored(device_id=%d boot_id=%d ts_boot_us=%d can_id=%d data=%s) "
                                "incoming(device_id=%d boot_id=%d ts_boot_us=%d can_id=%d data=%s)",
                                device_tag,
                                seqno,
                                stored.device_id,
                                stored.boot_id,
                ...
                cursor.execute(
                    ...
                    WHERE device_tag_id=?
                    """,
                    (max_incoming_seqno, max_incoming_seqno, device_uid, device_tag_id),
                )
                ...
                cursor.execute(
                    "SELECT last_seqno FROM device_tags WHERE device_tag_id=?",
                    (device_tag_id,),
                )
                row = cursor.fetchone()
                if row is None:
                    LOGGER.error(
                        "Unable to read updated last_seqno for device_tag=%r device_tag_id=%d",
                        device_tag,
                        device_tag_id,
                    )
                ...
                LOGGER.info(
                    "Commit transaction done: device_tag=%r device_uid=%d inserted=%d mismatches=%d last_seqno=%d",
                    device_tag,
                    device_uid,
                    inserted_count,
                    mismatch_count,
                ...
                LOGGER.error(
                    "SQLite error while committing records: device_tag=%r device_uid=%d",
                    device_tag,
                    device_uid,
                    exc_info=True,
                )
                ...
                LOGGER.critical(
                    "Unexpected exception while committing records: device_tag=%r device_uid=%d",
                    device_tag,
                    device_uid,
                    exc_info=True,
                )
    ...
    def get_device_tags(self) -> Iterable[str]:
        with self._lock:
            LOGGER.debug("Fetching all known device tags")
            cursor = self._connection.cursor()
            cursor.execute("SELECT device_tag FROM device_tags ORDER BY device_tag COLLATE NOCASE ASC")
            result = [str(row[0]) for row in cursor.fetchall()]
            LOGGER.info("Fetched %d device tags", len(result))
            return result
    ...
    def get_boots(
        ...
        with self._lock:
            LOGGER.debug(
                "Fetching boots for device_tag=%r earliest_commit=%r latest_commit=%r",
                device_tag,
                earliest_commit,
                latest_commit,
            )
            cursor = self._connection.cursor()
            device_tag_id = self._get_device_tag_id(cursor=cursor, device_tag=device_tag)
            if device_tag_id is None:
                LOGGER.info("No data for unknown device_tag=%r", device_tag)
                return []
            ...
            LOGGER.info(
                "Fetched %d boots for device_tag=%r earliest_commit=%r latest_commit=%r",
                len(out),
                device_tag,
                earliest_commit,
                latest_commit,
    ...
    def get_records(
        ...
        if seqno_min is not None and seqno_max is not None and seqno_min > seqno_max:
            LOGGER.warning(
                "Requested invalid seqno range for device_tag=%r: seqno_min=%d > seqno_max=%d",
                device_tag,
                seqno_min,
                seqno_max,
            )
        ...
        if not unique_boot_ids:
            LOGGER.info("No boot IDs requested for device_tag=%r", device_tag)
            return []

        with self._lock:
            LOGGER.debug(
                "Fetching records for device_tag=%r boot_ids=%s seqno_min=%r seqno_max=%r",
                device_tag,
                unique_boot_ids,
                seqno_min,
                seqno_max,
            ...
            for boot_id_chunk in _chunked(unique_boot_ids, self._SQL_VARIABLE_CHUNK):
                ...
                query = (
                    ...
                    "FROM can_frames "
                    f"WHERE device_tag_id=? AND boot_id IN ({placeholders})"
                )
                parameters: list[int] = [device_tag_id, *boot_id_chunk]
            ...
            LOGGER.info(
                "Fetched %d records for device_tag=%r boots=%d seqno_min=%r seqno_max=%r",
                len(result),
                device_tag,
                len(unique_boot_ids),
                seqno_min,
    ...
    def _initialize_schema(self) -> None:
        with self._lock:
            ...
            cursor.executescript("""
                -- Tag dictionary for deduplication and metadata.
                -- `last_seqno` is a write-through cache to avoid expensive MAX(seqno) lookups on huge tables.
                -- `last_device_uid` is optional observability state:
                -- it enables cheap detection of tag->UID drift and reflects heartbeat commits.
                CREATE TABLE IF NOT EXISTS device_tags
                (
                    device_tag_id    INTEGER PRIMARY KEY,
                    device_tag       TEXT    NOT NULL UNIQUE,
                    last_seqno       INTEGER NOT NULL DEFAULT 0,
                    last_device_uid  INTEGER
                );
                ...
                (
                    ...
                    device_tag_id INTEGER NOT NULL,
                    ts_boot_us    INTEGER NOT NULL,
                    boot_id       INTEGER NOT NULL,
                    seqno         INTEGER NOT NULL,
                    can_id        INTEGER NOT NULL,
                    data          BLOB    NOT NULL CHECK (length(data) <= 64),
                    FOREIGN KEY (device_tag_id) REFERENCES device_tags(device_tag_id),
                    -- Server-side idempotency key for uploader retries.
                    UNIQUE (device_tag_id, seqno)
                );

                -- Boot and seqno-oriented retrieval path.
                CREATE INDEX IF NOT EXISTS can_frames_tag_boot_seq
                    ON can_frames (device_tag_id, boot_id, seqno);

                -- Commit-time window filtering path.
                CREATE INDEX IF NOT EXISTS can_frames_tag_commit
                    ON can_frames (device_tag_id, commit_ts);

                -- Combined commit-time + boot aggregation path.
                CREATE INDEX IF NOT EXISTS can_frames_tag_boot_commit
                    ON can_frames (device_tag_id, boot_id, commit_ts);
                """)
            self._connection.commit()
            LOGGER.info("SQLite schema is ready")

    def _ensure_device_tag(
        self, cursor: sqlite3.Cursor, device_uid: int, device_tag: str
    ) -> tuple[int, int, int | None]:
        LOGGER.debug("Ensuring device tag row exists for %r", device_tag)
        cursor.execute(
            "INSERT OR IGNORE INTO device_tags (device_tag, last_device_uid) VALUES (?, ?)",
            (device_tag, device_uid),
        )
        cursor.execute(
            "SELECT device_tag_id, last_seqno, last_device_uid FROM device_tags WHERE device_tag=?",
            (device_tag,),
        )
        row = cursor.fetchone()
        if row is None:
            LOGGER.error("Failed to load/create device tag row for %r", device_tag)
            raise RuntimeError(f"failed to create or fetch device tag row for {device_tag!r}")
        resolved_uid: int | None = None if row[2] is None else int(row[2])
        return int(row[0]), int(row[1]), resolved_uid

    def _get_device_tag_id(self, cursor: sqlite3.Cursor, device_tag: str) -> int | None:
        cursor.execute("SELECT device_tag_id FROM device_tags WHERE device_tag=?", (device_tag,))
        row = cursor.fetchone()
        if row is None:
            return None
        return int(row[0])
    ...
    def _get_rows_by_seqno(
        self, cursor: sqlite3.Cursor, device_tag_id: int, seqnos: Iterable[int]
        ...
        for seqno_chunk in _chunked(seqno_list, self._SQL_VARIABLE_CHUNK):
            ...
            query = (
                "SELECT seqno, device_id, ts_boot_us, boot_id, can_id, data "
                "FROM can_frames "
                f"WHERE device_tag_id=? AND seqno IN ({placeholders})"
            )
            cursor.execute(query, [device_tag_id, *seqno_chunk])
            for row in cursor.fetchall():
...
class _DatabaseTests(unittest.TestCase):
    ...
    def test_commit_and_get_device_tags(self) -> None:
        records = [
            _make_record(1, boot_id=100, data=b"\x01"),
            _make_record(2, boot_id=100, data=b"\x02"),
        ]
        latest = self.db.commit(device_uid=1234, device_tag="alpha", records=records)
        self.assertEqual(2, latest)
        self.assertEqual(["alpha"], list(self.db.get_device_tags()))
    ...
    def test_commit_is_idempotent_for_duplicates(self) -> None:
        ...
        first = self.db.commit(device_uid=99, device_tag="alpha", records=records)
        second = self.db.commit(device_uid=99, device_tag="alpha", records=records)
    ...
    def test_duplicate_mismatch_logs_warning_and_keeps_existing(self) -> None:
        ...
        self.db.commit(device_uid=42, device_tag="alpha", records=[original])
        with self.assertLogs(__name__, level="WARNING") as captured:
            latest = self.db.commit(device_uid=42, device_tag="alpha", records=[conflicting])
    ...
    def test_empty_commit_returns_cached_last_seqno(self) -> None:
        self.assertEqual(0, self.db.commit(device_uid=555, device_tag="beta", records=[]))
        self.assertEqual(["beta"], list(self.db.get_device_tags()))

        self.db.commit(device_uid=555, device_tag="beta", records=[_make_record(7, boot_id=1)])
        self.assertEqual(7, self.db.commit(device_uid=555, device_tag="beta", records=[]))
    ...
    def test_get_records_filters_and_orders(self) -> None:
        ...
        self.db.commit(device_uid=1, device_tag="alpha", records=records)
    ...
    def test_get_boots_uses_overlap_time_window(self) -> None:
        ...
        cursor.execute(
            "UPDATE can_frames SET commit_ts=? WHERE device_tag_id=(SELECT device_tag_id FROM device_tags WHERE device_tag=?) AND seqno=1",
            (_datetime_to_epoch_seconds(datetime(2024, 1, 1, 0, 0, 0)), "alpha"),
        )
        cursor.execute(
            "UPDATE can_frames SET commit_ts=? WHERE device_tag_id=(SELECT device_tag_id FROM device_tags WHERE device_tag=?) AND seqno=2",
            (_datetime_to_epoch_seconds(datetime(2024, 1, 1, 2, 0, 0)), "alpha"),
        )
        cursor.execute(
            "UPDATE can_frames SET commit_ts=? WHERE device_tag_id=(SELECT device_tag_id FROM device_tags WHERE device_tag=?) AND seqno=10",
            (_datetime_to_epoch_seconds(datetime(2024, 1, 2, 0, 0, 0)), "alpha"),
        )
        cursor.execute(
            "UPDATE can_frames SET commit_ts=? WHERE device_tag_id=(SELECT device_tag_id FROM device_tags WHERE device_tag=?) AND seqno=11",
    ...
    def test_file_backed_database_persists_data(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first.commit(device_uid=101, device_tag="persist", records=[_make_record(7, boot_id=1, data=b"\x77")])
            first.close()

            second = SqliteDatabase(str(db_path))
            try:
                self.assertEqual(7, second.commit(device_uid=101, device_tag="persist", records=[]))
    ...
    def test_database_abstract_methods_raise_not_implemented(self) -> None:
        ...
        with self.assertRaises(NotImplementedError):
            _ = list(Database.get_device_tags(probe))
        with self.assertRaises(NotImplementedError):
        ...
        self.assertEqual([], list(probe.get_device_tags()))
        self.assertEqual([], list(probe.get_boots("x", None, None)))
        self.assertEqual([], list(probe.get_records("x", [], None, None)))

    def test_commit_rejects_empty_device_tag(self) -> None:
        with self.assertRaisesRegex(ValueError, "device_tag must be a non-empty string"):
            self.db.commit(device_uid=1, device_tag="", records=[_make_record(1)])

    def test_commit_logs_uid_change_and_duplicate_seqnos_in_same_payload(self) -> None:
        self.db.commit(device_uid=100, device_tag="alpha", records=[_make_record(1)])

        duplicate = _make_record(2, boot_id=1, ts_boot_us=22, can_id=0x123, data=b"\x22")
        with self.assertLogs(__name__, level="WARNING") as captured:
            latest = self.db.commit(device_uid=101, device_tag="alpha", records=[duplicate, duplicate])
        self.assertEqual(2, latest)
        ...
        with patch.object(self.db, "_get_rows_by_seqno", return_value={}):
            with self.assertLogs(__name__, level="CRITICAL") as captured:
                with self.assertRaisesRegex(RuntimeError, "missing stored row"):
                    self.db.commit(device_uid=1, device_tag="alpha", records=[_make_record(1)])
    ...
    def test_commit_missing_last_seqno_row_raises_runtime_error(self) -> None:
        ...
        cursor.executescript("""
            CREATE TRIGGER IF NOT EXISTS _test_delete_device_tags_after_update
            AFTER UPDATE ON device_tags
            BEGIN
                DELETE FROM device_tags WHERE device_tag_id=NEW.device_tag_id;
            END;
            """)
        self.db._connection.commit()
        with self.assertLogs(__name__, level="CRITICAL") as captured:
    ...
    def test_commit_sqlite_error_path_runs_rollback(self) -> None:
        with patch.object(self.db, "_ensure_device_tag", return_value=(999_999, 0, None)):
    ...
    def test_get_boots_unknown_device_tag_returns_empty(self) -> None:
    ...
    def test_get_records_unknown_device_tag_returns_empty(self) -> None:
        self.assertEqual([], list(self.db.get_records("unknown", [1], None, None)))

    def test_ensure_device_tag_raises_if_row_missing(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "failed to create or fetch device tag row"):
            self.db._ensure_device_tag(_CursorWithoutRows(), 1, "alpha")  # type: ignore[arg-type]

    def test_get_device_tag_id_returns_none_when_missing(self) -> None:
        class _CursorWithoutRows:
            def execute(self, *_args: object, **_kwargs: object) -> None:
                return None
        ...
        self.assertIsNone(self.db._get_device_tag_id(_CursorWithoutRows(), "alpha"))  # type: ignore[arg-type]
